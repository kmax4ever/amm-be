...
import { DexSyncHandler } from 'src/modules/dex/dexSyncHandler.service';
import { DexOrder } from 'src/modules/dex/models/dexOrder.entity';
...
export class SyncCoreService {
  ...
  async sync() {
    let session;
    try {
      const lastestBlock = (await this.getlastestBlockFromRpc()) - 10;
      if (lastestBlock <= 0) {
